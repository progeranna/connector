...
This is a strictly read-only full product acceptance checkpoint after integration of MP18R, MP20R, Checkpoint 2R R1 reachability recovery, and Checkpoint 2R R2 All Markets Back-focus recovery.

It requires local command execution, isolated production services, real-browser automation, exact focus verification, redirect/modal-state verification, and deterministic screenshot evidence.
...
Before any validation, fetch the authenticated remote and prove `origin/refactor/dashboard-modules` still resolves exactly to the accepted commit/tree. Any drift is a hard blocker.
...
- final merge tree: `495d14862b0996766b5376358b99382124df9916`
- frontend PR CI: 44 files / 614 tests
- bundle PR CI: 389599 bytes initial/largest/total under unchanged budgets
...
- the R1 worktree;
- the previous Checkpoint 2R validation worktrees;
- the R2 implementation worktree;
- `p35-manual-qa`.
...
- edit favicon/static assets during validation;
- begin Bridge 01, Bridge 02, Wave 4, or any later phase.

Transient ignored/generated build artifacts are permitted only as required to run validation. Remove them before final cleanliness verification.
...
All prescribed commands must pass. Declared service-dependent ignored tests may remain ignored exactly as designed; do not reinterpret ignored integration tests as executed success.

## Identity, cleanliness, and immutable-file audit

Before and after all validation record and compare:
...
For Live, validate the actual available/unavailable state returned by the product/backend. Never substitute Demo data into Live.
...
Explicitly prove on real deterministic Demo data before treating the matrix as passable:

- backend-provided Demo collection contains the actual accepted markets and anomalies;
- Market Health `View all` is visible, enabled, and keyboard focusable whenever the market collection is non-empty, including the accepted 7-market cardinality;
- Recent Anomalies `View all` is visible, enabled, and keyboard focusable whenever the anomaly collection is non-empty, including the accepted 3-anomaly cardinality;
- All Markets actually opens;
- All Anomalies actually opens;
- no artificial records are injected to force the actions to appear.
...
- direct All Markets initial focus behavior remains correct;
...
## Console/page-error acceptance and favicon re-audit

Require for the accepted full checkpoint:

- zero unexpected browser console errors;
- zero page errors;
- zero unhandled promise rejections.

Previous local browser runs recorded one automatic request to `/favicon.ico` returning HTTP 404. That observation was deliberately excluded from R2 product scope because it was non-causal to focus restoration.

This full checkpoint MUST re-audit it from a fresh browser context.

Rules:

1. Do not pre-classify the favicon observation as acceptable simply because it was previously seen.
2. Record whether `/favicon.ico` is requested and whether it produces a browser console error in the fresh production-preview run.
3. If the fresh run has a console error that violates the zero-unexpected-console-error acceptance rule, the checkpoint is blocked.
4. Do not fix the favicon/static asset in this validation worker.
5. On such a blocker, record the smallest separate proposed recovery lease and evidence; do not combine it with other work.

Warnings intentionally emitted by test tooling outside the real-browser acceptance surface must be reported separately from browser console errors.
...
5. Capture relevant screenshots and logs.
6. Classify the defect as MP18R, MP20R, R1 reachability, R2 focus recovery, pre-existing, environmental, or a separately observed issue.
...
`signalguard-rs/phase-3/reports/P3-CHECKPOINT-2R-RERUN-AFTER-R2/cbf5c543ada8752c273fbb2e91be029c9febc3d3.md`

Connector commit message:

`docs(phase-3): publish Checkpoint 2R rerun after R2`
...
`docs(phase-3): publish Checkpoint 2R rerun after R2 blocker`
...
- all command gates with test counts;
- immutable-file and cleanup audit;
- accepted R2 PR/CI identity read-back;
...
- console/page-error/unhandled-rejection counts, including explicit fresh favicon re-audit;
...
`P3_CHECKPOINT_2R_RERUN_AFTER_R2_COMPLETE`

only if every command gate, every mandatory browser obligation, R1/R2 regression gate, zero-error acceptance, screenshot requirement, final cleanliness check, and connector report publication succeeds.

Otherwise return exactly:

`P3_CHECKPOINT_2R_RERUN_AFTER_R2_BLOCKED`

Do not begin Bridge 01, Bridge 02, Wave 4, favicon/static-asset repair, or any later work.